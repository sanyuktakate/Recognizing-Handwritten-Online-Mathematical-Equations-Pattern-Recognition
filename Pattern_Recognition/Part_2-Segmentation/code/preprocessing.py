...
def process_stroke(stroke):
    '''
    Process a single stroke(used in processing strokes for the segmenter)
    :param stroke: a np array 2 dimensional
    :return:
    '''

    strokes = [{'x': stroke[:,0], 'y': stroke[:,1]}]
    ...
    res_strokes = np.vstack(strokes[0]['x'])
    y = np.vstack(strokes[0]['y'])

    res_strokes = np.append(res_strokes, y, axis=1)


    return res_strokes
    #
    # stroke[:, 0] = strokes[0]['x']
    # stroke[:, 1] = strokes[0]['y']
...
def normalize(strokes):
    ...
    #
    # y between [0,100] keeping the same aspect ratio
    #
    ...
    for stroke in strokes:
        ...
        for i in range(n):
            ...
            x_new.append(x_norm * 100)
            y_new.append(y_norm * 100)
